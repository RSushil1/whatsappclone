  ...
  return (
    <div>
      <div className='h-[90vh] m-5 justify-center shadow-2xl bg-white rounded-lg p-3 overflow-y-auto'>
        ...
        <div><img className='rounded-lg w-full bg-blue-500 h-[30vh]' src={`/api/auth/profile-coverPhoto/${id}`} alt={profile.name} /></div>
        <div className='flex flex-row m-3'>
          <div className='w-[20%]'><img className='h-32 w-32 rounded-full' src={`/api/auth/profile-photo/${id}`} alt={profile.name} />
          {auth?.user._id === id ?
