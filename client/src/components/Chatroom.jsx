                ...
                <div className=' h-screen'>
                    ...
                    <div className='h-[80vh] z-10 p-5 overflow-y-auto align-text-bottom'>
                        {/* {messages.map((message, index) => (
                            <div key={index} className={`w-full flex justify-${message.sender === auth?.user._id ? 'end' : 'start'}`} id='chatDiv'>
                                <div className={`flex flex-col place-items-${message.sender === auth?.user._id ? 'end' : 'start'}`}>
                                    <div className={`w-[80vh] flex flex-row ps-2 pt-1 pb-1 rounded-lg shadow-xl m-1 ${message.sender === auth?.user._id ? 'bg-red-200' : ' bg-orange-200'} ${message.sender === auth?.user._id ? 'mr-1' : 'ml-1'}`}>
                                        <div className="w-[80%] break-words text-ellipsis font-serif">
                                            {message.content}
                                        </div>
                                        <div className='w-[20%] ms-1'>
                                            <p className='flex flex-wrap text-[10px]'>{message.timestamp}</p>
                                        </div>
                                    </div>
                                    <div>
                        ...
                        {messages.map((message, index) => (
                            ...
                            >
                                <div
                                    className={`${message.sender === auth?.user._id ? 'bg-red-300 ml-3' : 'bg-gray-300 mr-3'} p-3 rounded-lg shadow-md max-w-[70%] flex flex-row`}
                                >
                                    <p className='text-sm break-words font-serif'>{message.content}</p>
                                    <p className='text-xs m-1 text-black'>{message.timestamp}</p>
                                </div>
                            </div>
                        ))}
