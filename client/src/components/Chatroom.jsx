...
const Chatroom = (props) => {
    ...
    const getMesssages = async () => {
        const res = await axios.get(`/api/auth/messages/${chat.chatId}`);
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            await axios.post(`/api/auth/messages/${chat.chatId}`, {
                    ...
                    <div className='flex flex-row justify-between text-white font-semibold p-2 bg-blue-900 z-10 h-[10vh]'>
                        <div className='ms-3'>
                            <div className="flex flex-row">
                                <div>
                                    {chat.user ?
                                        (<img className="w-12 h-12 ms-10 md:ms-1 rounded-full" src={`/api/auth/profile-photo/${chat.user.find(user => user.contactId._id !== auth?.user._id).contactId._id}`} alt="person" />) : (
